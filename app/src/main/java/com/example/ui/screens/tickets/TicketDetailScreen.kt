...
fun TicketDetailScreen(
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text("Campus QR Pass", fontWeight = FontWeight.Bold) },
                navigationIcon = {
                    IconButton(onClick = onNavigateBack, modifier = Modifier.testTag("btn_back_ticket_detail")) {
        ...
        if (ticket == null) {
            Box(
                ...
                Text("Ticket pass not found or cancelled.", color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
        } else {
            Column(
                modifier = Modifier
                ...
                TicketPassCard(
                    ...
                    onViewEmail = {
                        ...
                        if (email != null) {
                            viewingEmailLog = email
                        } else {
                            coroutineScope.launch {
                                snackbarHostState.showSnackbar("Digital verification slip registered on campus server.")
                            }
                        }
                    },
                    onCancelRegistration = {
                        currentUser?.let { user ->
                            coroutineScope.launch {
                                val res = repository.cancelRegistration(ticket.id, user)
                                if (res.isSuccess) {
                                    snackbarHostState.showSnackbar("Ticket cancelled.")
                                    onNavigateBack()
                                }
                            }
                        }
